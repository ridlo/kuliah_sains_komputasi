...
void recu(int ord){
    int orde=ord;
    for (int j=0;j<(ord-3);j++){
        int node1=0, node2=0, node3=0, node4=0, s=0;
        for (int i=0;i<gen;i++){
            i += gen;
            node1 = khromTerbaik[i%gen];
            node2 = khromTerbaik[(i+1)%gen];
            node3 = khromTerbaik[(i+orde-2)%gen];
            node4 = khromTerbaik[(i+orde-1)%gen];
            if ((jarak[node1][node2] + jarak[node3][node4]) > (jarak[node1][node3] + jarak[node2][node4])){
                s = khromTerbaik[(i+1)%gen];
                khromTerbaik[(i+1)%gen] = khromTerbaik[(i+orde-2)%gen];
                khromTerbaik[(i+orde-2)%gen] = s;}
            i -= gen;}
        //cout << orde << " ";
        orde -= 1; 
        }
