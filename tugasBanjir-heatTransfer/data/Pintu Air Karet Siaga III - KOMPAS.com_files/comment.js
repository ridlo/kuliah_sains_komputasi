...
head.js(
				//,'http://stat.k.kidsklik.com/data/2k11/comment/js/jquery-1.6.1.min.js'
				'http://assets.kompas.com/data/2k11/comment/js/jquery.scrollTo-1.4.2-min.js'
				,'http://assets.kompas.com/data/2k11/comment/js/jquery.cookie.js'
				,'http://assets.kompas.com/data/2k11/comment/js/custom.js'
				,'http://assets.kompas.com/data/2k11/comment/easyXDM/easyXDM.min.js'
			 , function () {
	var curURL=jQuery(location).attr('href');
	var curTitle=escape(document.title);
	var curToken=(jQuery.cookie('IFA')!=null)?jQuery.cookie('IFA'):'';
	jQuery.getJSON('http://api.kompas.com/widget/get_form_comment/'+curToken+'?jsoncallback=?',{token:curToken,articleID:articleID,url:curURL,title:curTitle},function(data){
		//s:xdm
		easyXDM.DomHelper.requiresJSON("http://stat.k.kidsklik.com/data/2k11/comment/easyXDM/json2.js");
		// this is really what you are after
		var xhrEXDM = new easyXDM.Rpc({
			remote: "http://api.kompas.com/plugins/easyXDM/cors/",
		}, {
			remote: {
				request: {}
			}
		});
		jQuery("#my_comment_submit_button").live('click',function(){
			if(! jQuery('#my_comment_info').length) {
				jQuery('#my_comment_content').parent().prepend('<span id="my_comment_info"></span><br/>');
			}
			var $t = jQuery(this);
			$t.attr('disabled', true);
			$t.next().attr('disabled', true);
			jQuery('#my_comment_content').attr('disabled', true);
			var comment    = jQuery('#my_comment_content').val();
			var submit_url = jQuery(this).parents('form:eq(0)').attr('action');
			var curTitle  = escape(jQuery('#frm_title').val());
			var curURL  = jQuery('#frm_url').val();
			var user_email  = jQuery('#frm_email').val();
			var user_name  = jQuery('#frm_name').val();
			if(jQuery.trim(comment).length) {
				xhrEXDM.request({
					url: "http://api.kompas.com/widget/article_comment",
					method: "POST",
					data: {txtID:articleID,comment:comment,parentid:0,title:curTitle,url:curURL,email:user_email,name:user_name}
				},function(rpcdata){
					var json = easyXDM.getJSONObject().parse(rpcdata.data);
					$t.attr('disabled', false);
					$t.next().attr('disabled', false);
					if(json.status==1){
						jQuery('#my_comment_content').attr('disabled', false).val('');
						jQuery('#my_comment_info').html('Terima kasih, komentar Anda telah kami terima.<br />');
					}else{
						jQuery('#my_comment_content').attr('disabled', false);
						jQuery('#my_comment_info').html('Mohon maaf, harap ulangi kembali.<br />');
					}
				},function(){
					$t.attr('disabled', false);
					$t.next().attr('disabled', false);
		...
		jQuery("#my_comment_reply_submit").live('click',function(){
			var comment    = jQuery('#my_reply_content').val();
			var submit_url = jQuery('#my_comment_form').attr('action');
			//var curTitle  = escape (jQuery('#frm_title').val());
			var curTitle	= escape (jQuery('title').val());
			var curURL  = jQuery('#frm_url').val();
			var user_email  = jQuery('#frm_email').val();
			var user_name  = jQuery('#frm_name').val();
			var parent_id  = jQuery('#my_comment_form_replay_parentid').val();
			parent_id = parent_id.split('_');
			if(jQuery.trim(comment).length) {
				xhrEXDM.request({
					url: "http://api.kompas.com/widget/article_comment",
					method: "POST",
					data: {txtID:articleID,comment:comment,parentid:parent_id,title:curTitle,url:curURL,email:user_email,name:user_name}
				},function(rpcdata){
					var json = easyXDM.getJSONObject().parse(rpcdata.data);
					if(json.status==1){
						var r = Math.floor(Math.random() * 25);
						jQuery('#my_reply_content').parents('div:eq(1)').html(jQuery('#my_reply_content').val());
						jQuery('#my_comment_reply_block').attr('id', 'my_comment_reply_block' + r);
					}else{
						jQuery('#my_reply_content').parents('div:eq(1)').html('Mohon maaf, harap ulangi kembali.<br />');
					}
				},function(){
					jQuery('#my_reply_content').parents('div:eq(1)').html('Mohon maaf, harap ulangi kembali.<br />');
				});
		...
		//s:xdm
		jQuery('#my_form_comment').html(data.results);
		if(articleID!=""){
			getComment(articleID,1);
			jQuery.mySubmitComment(articleID,curToken);
		}else{
			jQuery('#my_comment').html('<div class="c_font16 c_abu01_komen" align="center"><strong>Tidak ada komentar...</strong></div>');
		}
	});
});
