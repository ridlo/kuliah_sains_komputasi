...
double f(double x, double y){
    ...
    double ym = 100.0; // temperatur ruangan/ambient
    u = alpha*(y - ym);
    return u;}
...
int main(){
    ...
    a  = 0.0; // batas x kiri
    b  = 1.0; // batas x kanan
    ya = 800.0; // suhu di kiri
    yb = 400.0; // suhu di kanan
    ...
    do{
        ...
        y[j][0] = ya;
        if (j == 1){
            z[j][0] = z2;} // untuk tebakan kedua
        else{
            slope = (y[j-1][N-1] - y[j-2][N-1])/(z[j-1][0] - z[j-2][0]); // iterasi setelah tebakan 1 dan 2
