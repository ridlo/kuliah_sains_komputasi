...
double f(double t, double x){
    double Q = 1.; // debit masuk (m3/detik)
    double A = 10.; // luas tabung (m2)
    double a = 0.1; // luas lubang (m2)
    double g = 9.8; // gravitasi (m/s2) 
    double p = a/A;
    double q = Q/A;
    double c = sqrt((2.*g*p*p)/(1.-p*p));
    double dx;
    if (t < 600){
        dx = q - c*sqrt(x);}
    else if (t < 2500 && x > 0.0001){
        dx = 2.*q - c*sqrt(x);}
    else if (t >= 2500 && x > 0.0001){
        dx = - c*sqrt(x);}
    else {
