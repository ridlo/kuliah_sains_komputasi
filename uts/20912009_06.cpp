...
int main(){
    ...
    for (int i=0;i<MAX;i++){
        mutalism_selection(); // seleksi dengan elitism + mutasi
        for (int i=0;i<p;i++){x[i] = xi[i]; y[i] = yi[i];} // swap pop dg induk
        better(); // cek pop, siapa th ada yg terbaik, sebelum di mutasi lg
        bob();
...
void mutalism_selection(){ 
    // elistism -> membuat induk d yg dkt2 induk terbaik -> mutasi
    // kok jadi kayak SA -_-? 
    double prob;
    for (int i=0;i<p;i++){
        prob = unirand();
